...
static func get_distance_squared_from_rect_to_rect(
    ...
    var max_y := maxf(a.end.y, b.end.y)
    var inner_width := maxf(0.0, (max_x - min_x) - a.size.x - b.size.x)
    var inner_height := maxf(0.0, (max_y - min_y) - a.size.y - b.size.y)
...
static func does_rectangle_and_circle_intersect(
    ...
    var centers_distance_x := absf(circle_center.x - rectangle_center.x)
    var centers_distance_y := absf(circle_center.y - rectangle_center.y)
